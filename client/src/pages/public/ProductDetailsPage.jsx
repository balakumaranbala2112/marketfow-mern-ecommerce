...
function ProductDetailsPage() {
  ...
  if (!product) {
    return (
      <main className="mx-auto flex min-h-[60vh] max-w-7xl flex-col items-center justify-center px-6 py-12 text-center">
        ...
        <Link to={routePaths.products} className="mt-6 rounded-lg bg-primary-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-primary-700 shadow-sm shadow-primary-600/25">
          Browse Products
        </Link>
      </main>
    );
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      {/* Breadcrumb */}
      <Link to={routePaths.products} className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-primary-600 transition-colors">
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-2">
            ...
            <div className="mt-4 flex gap-3">
              {images.map((img, idx) => (
                <button
                  ...
                  className={`h-20 w-20 overflow-hidden rounded-lg border-2 transition-all ${idx === selectedImage ? "border-primary-500 ring-2 ring-primary-500/20" : "border-gray-200 hover:border-gray-300"}`}
                >
        ...
        <div>
          {product.category && (
            <Link to={`${routePaths.products}?category=${product.category._id}`} className="text-sm font-semibold text-primary-600 hover:text-primary-700 transition-colors">
              {product.category.name}
            </Link>
          )}
          <h1 className="mt-2 text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">{product.name}</h1>
          ...
          {inStock && (
            <div className="mt-6 flex flex-wrap items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg bg-primary-600 px-8 py-3 text-sm font-semibold text-white shadow-sm shadow-primary-600/25 transition-all duration-200 hover:bg-primary-700 hover:shadow-md disabled:opacity-60"
              >
          ...
          <div className="mt-8">
            ...
            <p className="mt-3 whitespace-pre-line text-sm leading-7 text-gray-600">{product.description}</p>
          </div>
              ...
              <dl className="mt-3 divide-y divide-gray-100">
                {product.specifications.map((spec, idx) => (
                  <div key={idx} className="flex py-2.5 text-sm">
                    <dt className="w-40 font-medium text-gray-500">{spec.key}</dt>
      ...
      <section className="mt-16 border-t border-gray-200 pt-12">
        ...
        {user && (
          <form onSubmit={handleSubmitReview} className="mt-6 rounded-xl border border-gray-200 bg-white p-6">
            ...
            <button
              ...
              className="mt-3 rounded-lg bg-primary-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm shadow-primary-600/25 hover:bg-primary-700 disabled:opacity-60 transition-all"
            >
        ...
        <div className="mt-8 space-y-6">
          {reviews.length === 0 ? (
            ...
            reviews.map((review) => (
              <div key={review._id} className="rounded-xl border border-gray-200 bg-white p-5">
